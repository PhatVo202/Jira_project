...
export const projectDetailReducer = (state = DEFAULT_STATE, action) => {
  ...
  switch (type) {
    ...
    case DELETE_PROJECT: {
      state.projectList = state.projectList.filter((item) => (item.id === payload ? false : true))
      break
    }
    ...
    case SET_MEMBER_INFO: {
      ...
      const index = dataUpdate.findIndex((item) => item.id === payload.projectId)
    ...
    case DELETE_MEMBER_PROJECT: {
      ...
      const idxMembers = arrMembers.findIndex((member) => member.userId === payload.userId)
    ...
    case DELETE_COMMENT: {
      state.commentList = state.commentList.filter((item) => (item.id === payload ? false : true))
      break
    }
